...
export default class BookWizard extends Application {
    ...
    async resaveBreadCrumbs(target) {
        ...
        await game.settings.set("dsk", "breadcrumbs", JSON.stringify(breadcrumbs))
    }
    ...
    async pinJournal(uuid, name = undefined) {
        ...
        game.settings.set("dsk", "breadcrumbs", JSON.stringify(breadcrumbs))
        this.render(true)
    }
    ...
    readBreadCrumbs() {
        let breadcrumbs = {}
        try {
            breadcrumbs = JSON.parse(game.settings.get("dsk", "breadcrumbs"))
        } catch (e) {
