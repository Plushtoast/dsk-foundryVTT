  ...
  class extends superclass {
    ...
    _onClickAction(event, target) {
      ...
      if (this.isEditable && handler) {
        ...
        ui.notifications.warn('DSKError.DamagePermission', { localize: true });
      } else {
    ...
    async _onDrop(event) {
      document.querySelectorAll('.window-content').forEach((el) => {
        el.classList.remove('dskDraggedOver');
      });
      super._onDrop(event);
    }
    ...
    _onDragOver(event) {
      ...
      if (hovered) {
        ...
        if (isInBorder) {
          hovered.classList.remove('dskDraggedOver');
        } else {
          hovered.classList.add('dskDraggedOver');
        }
      } else {
        document.querySelectorAll('.window-content').forEach((el) => {
          el.classList.remove('dskDraggedOver');
