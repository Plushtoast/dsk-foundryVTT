...
export default class ActorDSK extends Actor {
  ...
  maxDefenseValue() {
    ...
    for (let cur of this.items) {
      if (cur.type == "combatskill") {
        combatskills.push(CombatskillData._calculateCombatSkillValues(cur, this.system))
        ...
        const trait = TraitData._prepareMeleetrait(cur, this)
  ...
  prepareItems(sheetInfo) {
    ...
    for (let i = 1; i <= this.system.stats.schips.max; i++) {
      schips.push({
        value: i,
        cssClass: i <= this.system.stats.schips.value ? "fullSchip" : "emptySchip",
      });
    }
    ...
    for (let i = 0; i < this.items.contents.length; i++) {
      const it = this.items.contents[i];
      if (it.type == "equipment" && it.system.category == "bags") {
        containers.set(it._id, []);
      }

      preparedItems.push(it.system.prepareEmbeddedItemSheet());
    }

    preparedItems.sort((a, b) => { return a.name.localeCompare(b.name); });
    ...
    for (let j = 0; j < preparedItems.length; j++) {
      ...
      try {
        const parent_id = getProperty(i, "system.parent_id");
        ...
        switch (i.type) {
          ...
          case "poison":
            ...
            break;
          case "equipment":
            inventory[i.system.category].items.push(i);
    ...
    return {
      ...
      sheetLocked: this.system.sheetLocked,
      magic,
      allSkillsLeft: {
        body: skills.body,
