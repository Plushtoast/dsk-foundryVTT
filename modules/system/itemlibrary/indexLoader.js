export default class ItemLibraryIndexLoader {
  constructor({ debug = false } = {}) {
    this.debug = debug;
    this.workerName = "dsk.itemlibrary-index";
  ...
  async _ensureWorker() {
    ...
    try {
      ...
      console.warn("DSK | ItemLibraryIndexLoader: failed to create worker", err);
      this._enabled = false;
      this._worker = null;
      return null;
    }
  ...
  async _ensureFunctionsLoaded() {
    ...
    try {
      ...
      console.warn("DSK | ItemLibraryIndexLoader: failed to load worker functions", err);
      this._enabled = false;
      return false;
    }
  }
  ...
  async ensureIndex({ documentName, fields, fullTextSearch = false, token }) {
    ...
    const flexModuleUrl = getRoute
      ? getRoute("systems/dsk/libs/flexsearch.bundle.module.min.js")
      : new URL("systems/dsk/libs/flexsearch.bundle.module.min.js", baseUrl).toString();
    const flexIifeUrl = getRoute
      ? getRoute("systems/dsk/bundle/libs/flexsearch.bundle.iife.min.js")
      : new URL("systems/dsk/bundle/libs/flexsearch.bundle.iife.min.js", baseUrl).toString();
...
async function itemlibraryEnsureIndex(payload) {
  ...
  globalThis.__dskItemLibraryIndexState = globalThis.__dskItemLibraryIndexState || {
    indexes: {},
    config: {},
  };
  const state = globalThis.__dskItemLibraryIndexState;
