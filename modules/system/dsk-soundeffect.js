export default class DSKSoundEffect {
    ...
    static async loadSoundConfig() {
        const effectFile = await game.settings.get("dsk", "soundConfig")
        if (effectFile) {
            try {
                let file = await fetch(effectFile)
                let json = await file.json()
                this.sounds = json
                console.log("DSK | Sound Config Loaded")
            } catch (exception) {
        ...
        switch (item.type) {
            ...
            case "rangeweapon":
                paths = [
                    ...
                    ...successLevels.map(x => `${item.type }.${item.system.combatskill}.${action}_${x}`),
                    `${item.type }.${item.system.combatskill}.${action}`,
                    ...successLevels.map(x => `${item.type}.${item.system.combatskill}.default_${x}`),
                    `${item.type}.${item.system.combatskill}.default`
                ]
                break
            case "skill":
                paths = [
