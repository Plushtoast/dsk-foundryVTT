...
export default class TestSuite {
  static async #renderAll(documentType, hideAgain, onlyType, renderWorld, renderEmbedded = false) {
    const folder = await DSKUtility.getFolderForType(documentType, null, `${documentType} Test`);
    const items = {
      Item: game.items,
      Actor: game.actors,
  ...
  /**
   * Check that all data models are registered and match their templates
   * @param {string} templateJsonPath - Optional path to template.json
   */
  static async checkDataModels(templateJsonPath = undefined) {
    ...
    for (let documentName of ['Item', 'Actor']) {
      for (let type of types[documentName]) {
        if (!game.dsk.dataModels[documentName][type]) {
      ...
      if (templateJson) {
        for (let type of templateJson[documentName].types) {
          ...
          const objectFromModel = new game.dsk.dataModels[documentName][type]().toObject();
  ...
  /**
   * Build document types configuration for the system
   */
